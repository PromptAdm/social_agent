...
export function MarketingFooter() {
  return (
    <footer className="border-t border-[#1A1A24] px-5 pt-16 pb-10">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-[2fr_1fr_1fr_1fr_1fr] gap-8 mb-14">
          ...
          <div className="col-span-2 md:col-span-1">
            <Link href="/" className="inline-flex items-center gap-2.5 mb-4">
              <div className="w-7 h-7 bg-gradient-to-br from-indigo-500 to-violet-600 rounded-lg flex items-center justify-center flex-shrink-0 shadow-md shadow-indigo-500/20">
                <Hexagon className="w-[15px] h-[15px] text-white" strokeWidth={2.5} />
              </div>
              <span className="text-[15px] font-bold text-slate-100 tracking-tight">Nezora</span>
            </Link>
          ...
          {footerLinks.map(({ title, links }) => (
            <div key={title}>
              <h4 className="text-[10px] font-bold tracking-[0.1em] uppercase text-slate-600 mb-4">
                {title}
              </h4>
        ...
        {/* Bottom bar */}
        <div className="pt-6 border-t border-[#1A1A24] flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-[12px] text-slate-700">
            © {new Date().getFullYear()} Nezora. Todos os direitos reservados.
          </p>
          <div className="flex items-center gap-4">
            <Link href="#" className="text-[12px] text-slate-700 hover:text-slate-500 transition-colors">
              Termos
            </Link>
            <Link href="#" className="text-[12px] text-slate-700 hover:text-slate-500 transition-colors">
              Privacidade
