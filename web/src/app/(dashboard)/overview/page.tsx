...
function SectionHeader({ title, href, linkLabel }: { title: string; href: string; linkLabel?: string }) {
  return (
    <div className="flex items-center justify-between mb-4">
      <h2 className="text-[13px] font-semibold text-slate-700 tracking-tight">{title}</h2>
      <Link
        href={href}
        className="flex items-center gap-0.5 text-[11px] text-slate-500 hover:text-slate-700 transition-colors"
      >
        {linkLabel ?? 'Ver todos'}
            ...
            <div className="space-y-px">
              {pendingApproval.slice(0, 4).map((post) => (
                <div
                  ...
                  className="flex items-start gap-2.5 px-2 py-2 rounded-lg hover:bg-slate-100 transition-colors cursor-pointer"
                >
                  <div className="w-[18px] h-[18px] rounded-md bg-slate-100 border border-slate-300 flex items-center justify-center flex-shrink-0 mt-px">
                    <span className="text-[9px] font-semibold text-slate-500 uppercase">
                      {post.platform.slice(0, 2)}
                    </span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[13px] text-slate-600 truncate leading-snug">{post.caption}</p>
                    <p className="text-[11px] text-slate-600 mt-0.5">
